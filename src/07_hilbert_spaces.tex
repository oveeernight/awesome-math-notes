...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\forall x \in H \: \exists x \in X, y \in Y : h = x + y$
...
\begin{corollary}[критерий полноты системы элементов в гильбертовом пространстве]
    ...
    \[ \seq{x_\alpha}_{\alpha \in A} \text{ --- полное } \Rightarrow (y \perp x_{\alpha} \: \forall \alpha \in A \Rightarrow y = 0) \]
\end{corollary}
...
\begin{definition}[коэффициент Фурье]
    ...
    \begin{gather*}
        ...
        (x, e_n) \text{ --- коэффициент Фурье, если} \\
...
\begin{proof}
    \begin{gather*}
        ...
        \sum^n_{j=1} \abs{(x,e_j)} \leq 2 \: \forall n \in \bN \Rightarrow \\
...
\begin{theorem}[о разложении элемента гильбертова пространства в ряд Фурье]
    $H$ --- гильбертово, $x \in H, \seq{e_n}^\infty_{n=1}$ --- О.Н.С.
    \begin{enumerate}
...
\begin{example}
    $l^p, 1 \leq p < + \infty, e_n = (0, 0, \ldots, 0, \underbrace{n}, 0, \ldots)$
    \begin{gather*}
        x \in l^p, x = \seq{x_n}^\infty_{n=1} = \sum^\infty_{n=1} x_n e_n, \norm{x-s_n}_{l^p} \underset{n \to \infty}{\longrightarrow} 0 \\ 
        с_0, x \in c_0, \liml_{n \to \infty} x_n = 0 \quad x = \sum^\infty_{n=1} x_n e_n \quad \norm{x - s_n} \underset{n \to \infty}{\longrightarrow} 0
    \end{gather*}
\end{example}
...
\begin{remark}
    ...
    Собрались товарищи во Львове в кафе и составялил эти проблемы. Обычно математики любят сидеть в тиишине, нот вот Банах любил сидеть в кафе. Вероятно, они тамм не только чаи гоняли.
    Энфло (1973) сказал, что нет. Он предоставил множество контр-примеров.

    Сейчас в америке где-то работает.
\end{remark}
...
\begin{proof}
    \begin{gather*}
        ...
        \text{пусть } n \geq 2 \Rightarrow (\sigma_n, e_k) = \alpha_k \Rightarrow \alpha_k = (x, e_k)
    \end{gather*}
\end{proof}
...
По секрету, если убрать сепарабельность, то базис будет счётный. Какова размерность, такой и базис. Обычно, когда говорят о гильбертовом пространстве, подразумевают гильбертово сепарабельное.
\begin{proof}
    ...
    \end{gather*}
        ...
        \begin{gather*}
            ...
            L_n = \calL \seq{z_j}^n_{j=1}, L_n \subsetneq L_{n+1} \quad e_n = \frac{z_n - P_{L_{n-1}}} = \frac{z_n - \sum^{n=1}_{j=1} (z_n, e_j) e_j}{\ldots} \Rightarrow \\
            \seq{e_n}^\infty_{n=1} \text{ --- полная О.Н.С.} \Rightarrow \\
            \seq{e_n}^\infty_{n=1} \text{ --- базис (Шаудера)}2
        \end{gather*}
    \end{proof}
...
\begin{proof}
    ...
    Мы обсуждали, что линейный изоморфизм --- отношение эквивалентности, отношение изометричности --- тоже. Поэтому линейный изометрический изоморфизм есть отношение эквивалентности.
     ...
     \begin{gather*}
        ...
        \intertext{проверим, что $\varphi$ --- сюеръекция}
