...
\begin{proof}
    ...
    \end{gather*}
    Все, что надо~--- убедиться, что мера конечная.  Рассмотрим замыкание линейное оболчоки
    \begin{gather*}
        \begin{cases}
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        \left( \int_{A \setminus e} \mathbb{1} dx \right)^{\frac{1}{p}} + \left( \int_{A \setminus B} \mathbb{1} dx \right)^{\frac{1}{p}} < \varepsilon^{\frac{1}{p}} + \varepsilon^{\frac{1}{p}} = 2 \varepsilon^{\frac{1}{p}} 
    \end{multline*}
...
\begin{proof}
    ...
    \end{gather*}
    Тем самым мы доказали, что $\varphi(x)$ может быть приближена непрерывными функциями.
    Может быть, стоить отметить, что 
    $\mu G < \mu E + \varepsilon < p+ \infty \quad \int_X |\varphi(x)|^p d \mu - \int_G |\varphi(x)|^p d \mu < \mu(g) \Rightarrow
    \varphi \in L^p(X, \mu)$
