...
\begin{definition}[Метрика]
    $X$~--- множество. $\rho: X \times X \rightarrow \bR$, $\rho$~--- \textbf{метрика}, если при $\forall x, y, z \in X$ она обладает следующими свойствами 
    \begin{enumerate}
...
\begin{proof}[2 утверждение]
    Возьмём $\varepsilon > 0$, тогда по фундаментальности $\exists N  \: \forall n, m \in \bN \: ((\textcolor{blue}{\underline{n > N}} \, \land \, m > N) $ $\Rightarrow \underline{\rho(x_n, x_m)} < \varepsilon)$. Возьмём это $N$.

    $\exists a \liml x_{n_k} = a \Rightarrow \: \exists n_k (\textcolor{blue}{\underline{n_k > N}} \Rightarrow \rho(x_{n_k}, a) < \varepsilon)$. Возьмём это $n_k$.
...
\begin{proof}[3 утверждение]
    ...
    $\varepsilon_1: \exists n_1 \forall n, m  \in \bN \: ((n > n_1 \land m > n) \Rightarrow \rho(x_m,x_n) < \varepsilon_1)$. Выберем $n_1$, тогда $\forall m \in \bN (m > n_1 \Rightarrow \rho(x_m, x_{n_1}) < \varepsilon_1)$.
...
\begin{definition}[полунорма]
    Пусть $X$~--- линейное пространство над $\bR$ или $\bC$. Отображение $p: X \rightarrow \bR$ называется полунормой, если при $\forall x, y \in X \: \forall \lambda \in \bR (\bC)$
    \begin{enumerate}
...
\begin{proof}
    ...
    Пусть $\varepsilon > 0$. По критерию Коши $\exists N \in \bN  \, :\: \forall n, p \in \bN \: (n > N \Rightarrow \sum^{n+p}_{k=n+1} ||x_k|| < \varepsilon)$.
    \begin{multline*}
...
\begin{multline*}
    \forall \varepsilon > 0 \: \exists N \in \bN \: \forall m, n \in \bN \\
...
\begin{definition}
    $\bC^n, n \in \bN, l_n^\infty = \{x = (x_1, \ldots, x_n), x_j \in \bC \} $
